...
type LineInspectionHandler struct {
	service service.LineInspectionService
}

func NewLineInspectionHandler(s service.LineInspectionService) *LineInspectionHandler {
	return &LineInspectionHandler{service: s}
}

func (h *LineInspectionHandler) Register(group *gin.RouterGroup) {
...
func (h *LineInspectionHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *LineInspectionHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *LineInspectionHandler) create(c *gin.Context) {
	var input dto.CreateLineInspection
...
func (h *LineInspectionHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateLineInspection
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *LineInspectionHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *LineInspectionHandler) remove(c *gin.Context) {
